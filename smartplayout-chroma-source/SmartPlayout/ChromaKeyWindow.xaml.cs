 ...
 async void Refresh_Click(object s,RoutedEventArgs e)
 {
  try
  {
   if(IsNetworkSelection())
   {
    ...
    PreviewStatus.Text=$"OPENING {resolved.Provider} PREVIEW…";await _sourcePreviewCapture.StartNetworkAsync(NetworkFfmpeg(),resolved.PlayableUrl,ResolveNetworkAudio(),1280,720,25,resolved.UserAgent,resolved.Referer);return;
   }
